            ...
            <div class="product-img-bg min-h-80 flex items-center justify-center relative p-10 md:rounded-l-3xl overflow-hidden">
                @if(!empty($product['image_url']))
                    <img src="{{ $product['image_url'] }}"
                         ...
                         class="max-h-72 max-w-full object-contain drop-shadow-xl"
                         onerror="this.style.display='none';this.nextElementSibling.style.display='block'">
                    <span class="text-[130px] leading-none float-anim hidden">📱</span>
                @else
                    <span class="text-[130px] leading-none float-anim">📱</span>
                @endif
                ...
                <div class="absolute top-5 left-5 flex flex-col gap-2">
                    @if(($product['stock'] ?? 0) > 0)
                        <span class="badge badge-success">
                            <i class="fas fa-check text-[9px] mr-0.5"></i> Tersedia
                        </span>
                    @else
            ...
            <div class="p-6 md:p-10 flex flex-col justify-between">
                ...
                <div class="space-y-3 pt-4 border-t border-gray-100">
                    ...
                    @if(session('user') && session('user')['role'] == 'user')
                        ...
                        {{-- Form Tambah ke Keranjang (POST ke Laravel → forward ke Node.js) --}}
                        <form action="{{ route('cart.add') }}" method="POST" class="flex gap-3" id="cartForm">
                            @csrf
                            {{-- ✅ product_id dikirim ke backend --}}
                            ...
                            <div class="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden">
                                <button type="button" onclick="changeQty(-1)"
                                    class="px-3 py-2.5 hover:bg-gray-100 transition text-gray-600 font-bold select-none">
                                    &minus;
                                </button>
                                <input type="number" id="qtyDisplay"
                                    value="1" min="1" max="{{ $product['stock'] ?? 99 }}"
                        ...
                        {{-- Beli Sekarang — pakai addToCart() JS lalu redirect ke cart --}}
                        <button type="button"
                            onclick="buyNow({{ $product['id'] ?? 0 }})"
...
<script>
    ...
    function syncQty(val) {
        ...
        document.getElementById('qtyDisplay').value = v;
        hidden.value = v;
    }

    // ── Beli Sekarang: addToCart via API lalu redirect ke cart ──
    async function buyNow(productId) {
        const qty = parseInt(document.getElementById('qtyDisplay').value) || 1;
        await addToCart(productId, qty);
        // addToCart() sudah handle redirect jika sukses
    }
